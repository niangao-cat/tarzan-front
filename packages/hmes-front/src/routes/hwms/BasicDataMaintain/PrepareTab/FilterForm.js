...
class FilterForm extends Component {
  ...
  render() {
    ...
    const { tenantId, form, siteMap, enableFlagMap, preparingLeadTime } = this.props;
      ...
      <Form className={SEARCH_FORM_CLASSNAME}>
        <Row {...SEARCH_FORM_ROW_LAYOUT}>
          <Col {...FORM_COL_4_LAYOUT}>
            <Form.Item
              label={intl.get(`${modelPrompt}.preparingLeadTime`).d('备料提前期(min)')}
              {...SEARCH_FORM_ITEM_LAYOUT}
            >
              {<Input disabled value={preparingLeadTime} />}
            </Form.Item>
          </Col>
        </Row>
        <Row {...SEARCH_FORM_ROW_LAYOUT}>
