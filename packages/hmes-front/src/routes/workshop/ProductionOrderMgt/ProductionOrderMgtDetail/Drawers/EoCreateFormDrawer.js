...
import { Input, Modal, Form, InputNumber, Select, Tooltip, Icon, Row, Col, Spin } from 'hzero-ui';
...
export default class EoCreateFormDrawer extends Component {
  ...
  saveForm = () => {
    const { form, dispatch, workOrderId, onCancel, type } = this.props;

    form.validateFieldsAndScroll({ force: true }, (err, values) => {
      if (!err) {
        dispatch({
          type: 'productionOrderMgt/saveEoCreateForm',
          payload: {
            workOrderId,
            eoCount: values.eoCount,
            trxReleasedQty: values.trxReleasedQty,
            unitQty: values.unitQty,
          },
        }).then(res => {
          if (res && res.success) {
            this.props.onRefresh(workOrderId);
  ...
  render() {
    const {
      ...
      type,
      loading,
      productionOrderMgt: { eoMantissaOptions = [], eoCreateDetail = {}, controlTypeOptions = [] },
    } = this.props;
    ...
    const { enableEoMantissa, stateTrxReleasedQty } = this.state;
          ...
          >
            {getFieldDecorator('unitQty', {
              rules: [
                {
                  required: true,
                  message: intl.get(`${modelPrompt}.splitNotMsg`).d('单位EO数量不能为空'),
                },
                {
                  validator: (rule, value, callback) => {
                    if (stateTrxReleasedQty && value && stateTrxReleasedQty < value) {
                      callback(
                        intl.get(`${modelPrompt}.notUomLessMsg`).d('单位EO数量不能小于下达数量')
                      );
                    }
                    callback();
                  },
