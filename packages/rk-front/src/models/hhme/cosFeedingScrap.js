...
import {
  ...
  bindingEqConfirm,
  feedingBox,
} from '@/services/hhme/cosFeedingScrapService';

export default {
  namespace: 'cosFeedingScrap',
  state: {
    workcellInfo: {}, // 工位信息
    defaultSite: {},
    equipmentList: [], // 设备列表
  ...
  effects: {
    ...
    *getMaterialList({ payload }, { call, put }) {
      ...
      if (result) {
        ...
        for(let i=0; i<result.length; i++){
          result[i].qty = result[i].demandQty;
        }
        yield put({
          type: 'updateState',
          payload: {
    ...
    *setOut({ payload }, { call }){
    ...
    // 投料
    *feedingBox({ payload }, { call }){
      const res = getResponse(yield call(feedingBox, payload));
       return res;
    },
