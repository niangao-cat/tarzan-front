...
@connect(({ workOrderLossReport, loading }) => ({
  workOrderLossReport,
  fetchListLoading: loading.effects['workOrderLossReport/fetchList'],
...
export default class WorkOrderLossReport extends Component {
  ...
  init() {
    const { dispatch } = this.props;
    dispatch({
      type: 'workOrderLossReport/updateState',
      payload: {
        list: [],
        pagination: {},
      },
  ...
  handleFetchList(page = {}) {
    ...
    if(prodLineIds || workOrderNums || assemblyMaterialIds || planStartTimeFrom || planStartTimeTo) {
      ...
      dispatch({
        type: 'workOrderLossReport/fetchList',
        payload: {
          page: isEmpty(page) ? {
            ...page,
            pageSize: 20,
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Header title="工单损耗汇总报表">
          <ExcelExport
            exportAsync
            requestUrl={`/mes-report/v1/${tenantId}/work-order-attrition-sum/export`} // 路径
            otherButtonProps={{ type: 'primary' }}
            queryParams={this.handleGetFormValue()}
            disabled={this.handleGetFormDisabled()}
          />
        </Header>
        <Content>
