...
export async function getAntdLocale(language) {
  return import(`hzero-ui/es/locale-provider/${language.replace('-', '_')}.js`);
}
...
export async function queryIdpValue(lovCode) {
  ...
  return request(`${HZERO_PLATFORM}/v1/${tenantId}/lovs/value`, {
    query: {
      lovCode,
    },
  });
...
export async function queryMapIdpValue(params) {
  const tenantId = getCurrentOrganizationId();
  return request(`${HZERO_PLATFORM}/v1/${tenantId}/lovs/value/batch`, {
    query: params,
  });
}

export async function queryMapIdpValueForTenantId(params) {
  return request(`${HZERO_PLATFORM}/v1/${params.tenantId}/lovs/value/batch`, {
    query: params,
  });
}
...
export async function queryFileList(params) {
  const tenantId = getCurrentOrganizationId();
  return request(
    `${HZERO_FILE}/v1${isTenantRoleLevel() ? `/${tenantId}/` : '/'}files/${params.attachmentUUID
    }/file`,
    {
      method: 'GET',
      query: params,
...
export async function removeFileList(params) {
  return request(
    `${HZERO_FILE}/v1/files/delete-by-uuidurl/not-null?bucketName=${params.bucketName}&attachmentUUID=${params.attachmentUUID}`,
    {
      method: 'POST',
      body: params.urls,
    }
...
export async function removeUploadFile(params) {
  ...
  const tenantId = getCurrentOrganizationId();
  return request(
    `${HZERO_FILE}/v1${isTenantRoleLevel() ? `/${tenantId}/` : '/'}files/delete-by-url?bucketName=${params.bucketName
    }`,
    {
      method: 'POST',
...
export async function removeFile(params) {
  const tenantId = getCurrentOrganizationId();
  return request(
    `${HZERO_FILE}/v1${isTenantRoleLevel() ? `/${tenantId}/` : '/'
    }files/delete-by-uuidurl?bucketName=${params.bucketName}&attachmentUUID=${params.attachmentUUID
    }`,
    {
      method: 'POST',
      body: params.urls,
...
export async function queryColumn(params) {
  ...
  return request(`${params.requestUrl}`, requestParams);
}
...
export async function downloadFile(params = {}) {
  ...
  if (!isEmpty(queryParams)) {
    queryParams.forEach(item => {
      if (Object.keys(newQueryParams).includes(item.name)) {
        newQueryParams[item.name] = [].concat(newQueryParams[item.name], item.value);
      } else {
        newQueryParams[item.name] = item.value;
      }
    });
  }
  let query = {
  ...
  if (method === 'GET') {
    query = { ...query, query: { ...newQueryParams, ...queryData } };
  } else if (method === 'POST') {
    query = { ...query, body: queryData, query: newQueryParams };
  }
...
export async function queryTL(params) {
  const { className, fieldName, ...others } = params;
  return request(`${Host}/v1/hidden/multi-language?className=${className}&fieldName=${fieldName}`, {
    method: 'POST',
    body: { ...others },
  });
}
 ...
 */
export async function queryAttrTL(params) {
  const tenantId = getCurrentOrganizationId();
  return request(`${Host}/v1/${tenantId}/mt-extend-setting/lang/ui`, {
    method: 'GET',
    query: params,
  });
}
...
export async function queryStaticText(textCode, lang = getCurrentLanguage()) {
  ...
  return request(
    `${HZERO_PLATFORM}/v1/${organizationRoleLevel ? `${organizationId}/` : ''
...
export async function handleCheckMaterialCode(params) {
  const tenantId = getCurrentOrganizationId();
...
export async function initiateAsyncExport(params) {
  const { requestUrl, exportAsyncUrl, queryParams, method, queryData } = params;
  const responseBody = requestUrl ? {
    method,
  } : { method, responseType: 'text' };
  const url = requestUrl || exportAsyncUrl;
  let newUrl = !url.startsWith('/api') && !url.startsWith('http') ? `${API_HOST}${url}` : url;
...
export async function createAsyncExportTask(params) {
  const { createTaskUrl, queryParams, method, queryData } = params;
