...
import {
  ...
  changeEqConfirm,
  bindingEqConfirm,
} from '@/services/hmes/timeManagementService';
...
export default {
  ...
  effects: {
    ...
    *scanningOutFurnaceCode({ payload }, { call, put }) {
      ...
      if (result) {
        const isRecordLabCodeList = result.labCodeAndRemarkList ? result.labCodeAndRemarkList.map(ele => ele.labCode) : [];
        const routerStepRemarkList = result.labCodeAndRemarkList ? result.labCodeAndRemarkList.map(ele => ele.routerStepRemark) : [];
        yield put({
          type: 'updateState',
          payload: {
            isRecordLabCodeList,
    ...
    *handleFetchSnList({ payload }, { call, put }) {
      const result = getResponse(yield call(handleFetchSnList, payload));
      if (result) {
        yield put({
          type: 'updateState',
          payload: {
            snList: result,
            // snListPagination: createPagination(result),
          },
        });
      }
      return result;
