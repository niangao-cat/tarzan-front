...
export default class SerialItem extends Component {
  ...
  render() {
    const { itemList = [], loading, equipmentInfo = {}, deleteEqLoading, changeEqLoading, bindingEqLoading, bindingEqConfirmLoading, changeEqConfirmLoading, modelName } = this.props;
    const { visible, selectedRows, record } = this.state;
    const titleProps = {
      titleValue: '工位设备',
      used: equipmentInfo.completeQty,
      sum: equipmentInfo.totalQty,
    };
    ...
    return (
      <div className={modelName === 'operationPlatform' ? styles['station-item'] : styles['self-item']}>
        <Title {...titleProps} />
        <Spin spinning={loading || deleteEqLoading || bindingEqLoading || bindingEqConfirmLoading || changeEqConfirmLoading || false}>
          <div className={modelName === 'operationPlatform' ? styles['station-item-content'] : styles['station-item-content-new']}>
            <Form>
