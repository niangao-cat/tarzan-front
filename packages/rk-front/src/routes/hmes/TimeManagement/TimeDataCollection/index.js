...
@connect(({ timeManagement, loading }) => ({
  ...
  fetchDataCollectionLoading: loading.effects['timeManagement/fetchDataCollection'],
  saveDataCollectionLoading: loading.effects['timeManagement/saveDataCollection'],
}))
export default class TimeManagement extends Component {
  ...
  componentDidMount() {
    ...
    if (routerParam.code) {
      ...
      dispatch({
        type: 'timeManagement/fetchDefaultData',
        payload: {
  ...
  fetchDataCollection(fields = {}) {
    ...
    const { routerParamState } = this.state;
    dispatch({
      type: 'timeManagement/fetchDataCollection',
      payload: {
        ...fieldsValue,
        tagIdList,
  ...
  saveDataCollection() {
    const {
      dispatch,
      timeManagement: { dataCollection = [], dataCollectionPagination = {} },
    } = this.props;
    const params = getEditTableData(dataCollection);
    if (Array.isArray(params) && params.length !== 0) {
      dispatch({
        type: 'timeManagement/saveDataCollection',
        payload: params,
      }).then(res => {
        if (res) {
          this.setState({ editStatus: false });
