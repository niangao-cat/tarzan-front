...
export default class FirstProcessInfo extends Component {
  ...
  @Bind()
  handleOutSite(info = {}) {
    const { onOutSite, form: { getFieldValue } } = this.props;
    if (onOutSite) {
      onOutSite(info).then(res => {
        if (res) {
          this.handleAutoCreateSnNum(getFieldValue('workOrderId'));
        }
      });
    }
  }
  ...
  handleInitData() {
    ...
    form.resetFields(['snNumber', 'workOrderId']);
  }
  ...
  handleAutoCreateSnNum(workOrderId) {
    const { onAutoCreateSnNum, form: { setFieldsValue } } = this.props;
    if (onAutoCreateSnNum) {
      onAutoCreateSnNum(workOrderId).then(res => {
        if (res) {
          setFieldsValue({ snNumber: res.snNum });
        }
      });
    }
  }
        ...
        <div className={classNames(styles['operationPlatform_base-up'], styles['base-up'])}>
          <div className={styles['pre-form']}>
            <Row>
                ...
                <Form.Item label="工单号" labelCol={{ span: 7 }} wrapperCol={{ span: 17 }} style={{ marginLeft: '8px' }}>
                  {getFieldDecorator('workOrderId', {
                    ...
                    <Lov
                      ...
                      onChange={(val, data = {}) => {
                        if (val) {
                          this.handleAutoCreateSnNum(val);
                        } else {
                          this.handleInitData();
                        }
              ...
              <Col span={14}>
                <Form.Item label="物料序列号" labelCol={{ span: 7 }} wrapperCol={{ span: 17 }} className={styles['form-search-scanner']}>
                  <img src={scannerImageMat} alt="" className={styles['base-scanner']} />
                  {getFieldDecorator('snNumber', {
                    initialValue: baseInfo.snNumber,
                  })(<Input disabled className='operation-platform-sn-num' />)}
                </Form.Item>
              </Col>
              <Col span={10}>
                <div className={styles['base-button']}>
                  <Button type="primary" onClick={() => this.handleFetchBaseInfo()} className={styles['base-button-clean']}>
                    进站
                  </Button>
                </div>
              </Col>
