...
@connect(({ manufacturingCenterBoard }) => ({
      ...
      <Fragment>
        <TopBox>
          <div className='top_box'>
            ...
            <TimeBox>
              <h3 style={{ float: 'left', fontSize: '14px', marginRight: '8px' }}>{this.state.timeStr}</h3>
              <Button
                onClick={() => screenFull()}
                style={{ float: 'right', background: 'rgb(3, 5, 12)', color: '#bcdcff', borderColor: '#bcdcff' }}
                icon={isFullFlag ? 'shrink' : 'arrows-alt'}
                size="small"
              />
              <Button size="small" onClick={() => this.handleOpenModal()} style={{ color: '#bcdcff', background: 'rgb(3, 5, 12)', borderColor: '#bcdcff'}}>切换产线</Button>
            </TimeBox>
          </div>
        </TopBox>
      </Fragment>
