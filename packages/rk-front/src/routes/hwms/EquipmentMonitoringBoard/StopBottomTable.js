...
export default class StopMidTable extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: intl.get(`${commonModelPrompt}.position`).d('时长'),
        dataIndex: 'position',
        align: 'center',
      },
      {
        title: intl.get(`${commonModelPrompt}.doTime`).d('处理人'),
        dataIndex: 'doTime',
        align: 'center',
      },
      {
        title: intl.get(`${commonModelPrompt}.stopTime`).d('处理方式'),
        dataIndex: 'stopTime',
        align: 'center',
      },
    ];
      ...
      <React.Fragment>
        <Row>
          <div>
            ...
            <span style={{ fontSize: '1vw', fontWeight: 'bold', color: 'rgba(51,51,51,1)' }}>同异常类型最近三次</span>
          </div>
          <Table
            columns={columns}
            bordered
