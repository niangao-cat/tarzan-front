 ...
 * @Date: 2020-08-06 19:54:41
 * @LastEditTime: 2020-08-06 19:55:23
 */

import React, { Component } from 'react';
import { Button, Tooltip, Modal, Spin, Form, Row, Col, Table, Input, Checkbox, InputNumber } from 'hzero-ui';
...
export default class BatchMaterial extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  handleEnterClick(e, data) {
    ...
    const assetEncoding = e.target.value;
  ...
  handleBindingEq(e, record) {
    ...
    if(onBinding) {
      onBinding({
        ...
        workcellId,
        siteId,
        scanAssetEncoding: e.target.value,
      }).then(res => {
        if(res && res.success) {
          notification.success();
          this.setState({ visible: false });
  ...
  render() {
    const { itemList = [], loading, deleteEqLoading, changeEqLoading, bindingEqLoading, bindingEqConfirmLoading, changeEqConfirmLoading, selectData = [], setSelectData, changeMaterialQty } = this.props;
            ...
            {itemList.map((e, index) => {
              return (
                <div
                  ...
                  style={{
                    cursor: e.materialId ? 'pointer' : 'auto',
                    borderColor: this.handleChangeColor(e.color),
                  }}
                  onClick={() => this.handleOpenModal(e)}
                >
                  <span className={styles['martial-orderCheck']}>
                    <Checkbox
                      checkedValue={0}
                      unCheckedValue={1}
                      checked={e.checked}
                      onChange={() => {
                      if (selectData.length===0||selectData.map(item=>item.materialId).indexOf(e.materialId) === -1) {
                        selectData.push(e);
                        setSelectData(selectData, itemList);
                      } else {
                        const odd = selectData;
                        odd.splice(odd.map(item=>item.materialId).indexOf(e.materialId), 1);
                        setSelectData(odd, itemList);
                      }
                    }}
                    />
                  </span>
                  <div
                    ...
                    style={{ width: '70%', borderRight: 0 }}
                  >
                    <Tooltip title={`${e.materialCode}-${e.materialName}-${e.uomCode}`}>
                      <div
                        className={styles['martial-name']}
                        style={{
                          borderColor: this.handleChangeColor(e.color),
                          borderRightStyle: 'solid', borderRightColor: '#45acf1', borderRightWidth: '1px',
                        }}
                      >
                        {`${e.materialCode}-${e.materialName}-${e.uomCode}`}
                      </div>
                    </Tooltip>
                    <Tooltip title={`${e.materialLotCode}-${e.materialLotQty}`}>
                      <div
                      ...
                      >
                        {e.materialId ? `${e.materialLotCode}-${e.materialLotQty}` : (
                  ...
                  >
                    <Tooltip title={`${e.investedQty}/${e.demandQty}`}>
                      <div
                        className={styles['martial-name']}
                        style={{
                          borderColor: this.handleChangeColor(e.color),
                    ...
                    <Tooltip title={e.qty}>
                      ...
                      >
                        <span style={{ height: '20px'}}> <InputNumber onChange={qty=>changeMaterialQty(qty, index)} style={{textAlign: 'left', height: '99%'}} min="0" defaultValue={e.qty} /></span>
                      </div>
                    </Tooltip>
                  </div>
                </div>
