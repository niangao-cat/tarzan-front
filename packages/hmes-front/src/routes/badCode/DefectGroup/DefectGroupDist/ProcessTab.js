...
@connect(({ defectGroup, loading }) => ({
  defectGroup,
  loading: loading.effects['defectGroup/mtNcValidOperList'],
}))
@Form.create()
...
export default class ProcessTab extends PureComponent {
  ...
  deleteData = (record, index) => {
    ...
    if (record.ncValidOperId !== '') {
      dispatch({
        type: 'defectGroup/deleteProcessDispatch',
  ...
  render() {
    const {
      ...
      loading,
      canEdit,
      defectGroup: { mtNcValidOperList = [], limitSiteId },
    } = this.props;
