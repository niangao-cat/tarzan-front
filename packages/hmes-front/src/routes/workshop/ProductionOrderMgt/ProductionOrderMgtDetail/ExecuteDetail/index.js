...
export default class ExecuteDetail extends React.Component {
  ...
  refreshDetail = val => {
    ...
    history.push(`/hmes/workshop/production-order-mgt/execute-operation-management/detail/${val}/${workOrderId}`);
    dispatch({
      type: 'execute/updateState',
      payload: {
        routingList: [],
  ...
  render() {
    ...
    const basePath = match.path.substring(0, match.path.indexOf('/execute'));
    const eoId = match.params.id;
    const { workOrderId } = match.params;
    const {
      buttonFlag,
    ...
    return (
      <>
        <Header
          ...
          backPath={`${basePath}/detail/${workOrderId}`}
        >
          <>
            {buttonFlag ? (
              <Button
