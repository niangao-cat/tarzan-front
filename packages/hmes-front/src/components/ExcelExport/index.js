...
export default class ExcelExport extends React.Component {
  ...
  @Bind()
  showModal(flag) {
    const { defaultConfig, requestUrl, exportUrl } = this.props;
  ...
  handleDownloadFile(config = {}, params = {}) {
    ...
    downloadFile({
      ...
      if (res) {
        ...
        const lastName = this.props.fileName ? this.props.fileName.split('.')[1] : null;
  ...
  handleAsyncDownloadFile(params = {}) {
    const { requestUrl, exportAsyncUrl, queryParams, method = 'GET', createTaskUrl } = this.props;
    let queryData = queryParams;
