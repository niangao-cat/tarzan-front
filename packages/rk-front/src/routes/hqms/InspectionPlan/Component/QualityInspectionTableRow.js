...
export default class QualityInspectionTableRow extends Component {
  ...
  render() {
    ...
    const { loading, pagination, onSearch, dataSource, defectLevel = [], selectedHeadCache = [] } = this.props;
    const columns = [
      {
          ...
          ['update', 'create'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator(`processId`, {
                rules: [
                  {
                    required: selectedHeadCache.length>0?selectedHeadCache[0].inspectionType !== "OQC":false,
                    message: '工序必输',
                  },
                ],
      ...
      {
        title: intl.get(`${commonModelPrompt}.defectLevel`).d('检验频率'),
        width: 150,
        dataIndex: 'frequency',
        align: 'center',
        render: (val, record) =>
          ['update', 'create'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator(`frequency`, {
                rules: [
                  {
                    required: true,
                    message: '检验频率',
          ...
          ['update', 'create'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator(`inspectionTool`, {
                initialValue: record.inspectiomTool,
              })(
                <Lov
                  queryParams={{
                    tenantId: getCurrentOrganizationId(),
