...
@connect(({ barcodeQuery, loading }) => ({
  ...
  loading: {
    fetchLoading: loading.effects['barcodeQuery/queryBarcodeHistoryList'],
  },
}))
...
class BarcodeHistory extends Component {
  constructor(props) {
    ...
    this.state = {
      filterColum: [
        ...
        {
          fieldName: intl.get(`${modelPrompt}.eventTypeCode`).d('主表事件'),
          fieldKey: 'eventTypeCode',
          hidden: 0,
          orderSeq: 1,
        },
        {
          fieldName: intl.get(`${modelPrompt}.eventId`).d('主表事件id'),
          fieldKey: 'eventId',
          hidden: 0,
          orderSeq: 2,
        },
        {
          fieldName: intl.get(`${modelPrompt}.eventTime`).d('主表事件创建时间'),
          fieldKey: 'eventTime',
          hidden: 0,
          orderSeq: 3,
        },
        {
          fieldName: intl.get(`${modelPrompt}.eventBy`).d('主表事件创建人'),
          fieldKey: 'eventBy',
          hidden: 0,
          orderSeq: 4,
        },
        {
          fieldName: intl.get(`${modelPrompt}.attrEventTypeDesc`).d('扩展表事件'),
          fieldKey: 'attrEventTypeDesc',
          hidden: 0,
          orderSeq: 5,
        },
        {
          fieldName: intl.get(`${modelPrompt}.attrEventId`).d('扩展表事件id'),
          fieldKey: 'attrEventId',
          hidden: 0,
          orderSeq: 6,
        },
        {
          fieldName: intl.get(`${modelPrompt}.attrEventTime`).d('扩展表事件创建时间'),
          fieldKey: 'attrEventTime',
          hidden: 0,
          orderSeq: 7,
        },
        {
          fieldName: intl.get(`${modelPrompt}.attrEventBy`).d('扩展表事件创建人'),
          fieldKey: 'attrEventBy',
          hidden: 0,
          orderSeq: 8,
        },
        {
          fieldName: intl.get(`${modelPrompt}.trxPrimaryQty`).d('事务数量'),
          fieldKey: 'trxPrimaryQty',
          hidden: 0,
          orderSeq: 9,
        },
        {
        ...
        {
          fieldName: intl.get(`${modelPrompt}.actualLocatorName`).d('实际存储货位'),
          fieldKey: 'actualLocatorName',
          hidden: 0,
          orderSeq: 27,
        },
        {
        ...
        {
          fieldName: intl.get(`${modelPrompt}.effectiveDate`).d('有效期'),
  ...
  handleSearch(fields = {}) {
    ...
    const materialLotIds = parentItem.map(item => {
      return item.materialLotId;
    });
    dispatch({
      type: 'barcodeQuery/queryBarcodeHistoryList',
      payload: {
        tenantId,
        materialLotIds,
  ...
  render() {
    ...
    const {
      ...
      barcodeQuery: { historyList = [], hisPagination = {} },
    } = this.props;
    const columns = [
      {
      ...
      {
        title: intl.get(`${modelPrompt}.eventTypeDesc`).d('主表事件'),
        dataIndex: 'eventTypeDesc',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.eventId`).d('主表事件id'),
        dataIndex: 'eventId',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.eventTime`).d('主表事件创建时间'),
        dataIndex: 'eventTime',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.eventBy`).d('主表事件创建人'),
        dataIndex: 'eventBy',
        width: 120,
      },
      {
        title: intl.get(`${modelPrompt}.attrEventTypeDesc`).d('扩展表事件'),
        dataIndex: 'attrEventTypeDesc',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.attrEventId`).d('扩展表事件id'),
        dataIndex: 'attrEventId',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.attrEventTime`).d('扩展表事件创建时间'),
        dataIndex: 'attrEventTime',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.attrEventBy`).d('扩展表事件创建人'),
        dataIndex: 'attrEventBy',
        width: 150,
      },
      {
        title: intl.get(`${modelPrompt}.trxPrimaryQty`).d('事务数量'),
        dataIndex: 'trxPrimaryQty',
        width: 90,
      },
      {
      ...
      },
      {
        ...
        width: 100,
        className: styles['barcode-query-history-green'],
      },
      {
    ...
    return (
      <React.Fragment>
        <Header
          title={intl.get('hwms.barcodeQuery.view.message.barcodeHistory').d('条码历史')}
          backPath="/hwms/barcode/list"
        />
        <Content>
          <div className="barcode-query-history-tableClass">
            {this.renderDropDown()}
            <Table
              bordered
              ...
              scroll={{ x: this.tableScrollWidth(columns, 170) }}
