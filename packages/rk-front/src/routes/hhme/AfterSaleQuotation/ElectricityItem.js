...
const ElectricityItem = (props, ref) => {
  ...
  const handleCleanLine = (record) => {
    ...
    if (onCleanLine) {
      onCleanLine('electricLineList', 'materialId', record);
    }
  };
  ...
  const handleEditLine = (record, flag) => {
    ...
    if (onEditLine) {
      onEditLine('electricLineList', 'materialId', record, flag);
    }
  };
  ...
  const handleCreate = () => {
    ...
    if (onCreate) {
      onCreate('electricLineList', 'materialId');
    }
  };
  ...
  const handleDelete = (record) => {
    ...
    if (onDelete) {
      onDelete(record, 'electricLineList');
    }
  };
  ...
  const handleChangeMaterialCode = (val, data, record) => {
    ...
    if (!isEmpty(record)) {
      onFetchSendDate(data, record, 'electricLineList');
    }
  };
  ...
  let columns = [
    {
      title: !getFieldValue('electricNoFlag') && isEdit && (
        <Button
  ...
  return (
    <Fragment>
      <div className={styles['afterSaleQuotation_line-title']}>
        ...
        <div className={styles['afterSaleQuotation_header-title']}>电学器件</div>
        <div className={styles['afterSaleQuotation_form-title']}>
          <Form ref={formRef}>
            <Form.Item label="无需更换" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
              {form.getFieldDecorator('electricNoFlag', {
                initialValue: baseInfo.electricNoFlag === 'Y',
...
export default Form.create({ fieldNameProp: null })(forwardRef(ElectricityItem));