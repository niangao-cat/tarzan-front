...
@connect(({ inProcessQueryReport, loading }) => ({
  ...
  detailLoading: loading.effects['inProcessQueryReport/fetchDetailList'],
}))
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (val, record) => {
          if (record.standardTimer === null || record.standardTimer === undefined || record.standardTimer === "") {
            return val;
          } else if (record.timeDiff != null && Number(record.standardTimer) < Number(record.timeDiff)) {
      ...
      {
        title: '返修SN',
        dataIndex: 'reworkMaterialLotCode',
      },
      {
        title: '标准时长',
        dataIndex: 'standardTimer',
      },
      {
        title: '操作时间',
