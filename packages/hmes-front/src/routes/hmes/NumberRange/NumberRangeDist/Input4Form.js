...
export default class Input4Form extends Component {
  constructor(props) {
    super(props);
    props.onRef(this);
    this.state = {
  ...
  onRadioChange = e => {
    ...
    const {
      ...
      setCurrentRule,
      changeRuleId,
      numberRange: { inputBox4List = {} },
    } = this.props;
    const { numrangeRuleId } = inputBox4List;
    setCurrentRule(4, e.target.value);
    changeRuleId(e.target.value, 3);
    dispatch({
      type: 'numberRange/updateState',
      payload: {
        inputBox4List: { numrangeRuleId },
      },
    });
    if (e.target.value === '1') {
      form.resetFields([
  ...
  onInputChange(rule, value, exampleValue, format) {
    ...
    setCurrentValue(4, value, exampleValue, format);
    if (rule === '5') {
