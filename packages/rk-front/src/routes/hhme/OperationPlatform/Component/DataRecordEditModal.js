...
export default class DataRecordModal extends Component {
  ...
  @Bind()
  handleCreate() {
    const { onCreate, baseInfo } = this.props;
    if (onCreate) {
      onCreate('addDataRecordList', 'jobRecordId', { isSupplement: '1', groupPurpose: 'DATA', jobId: baseInfo.jobId });
    }
  }
    ...
    if (onEnterClick && value) {
      record.$form.validateFields((err, val) => {
        if(!err) {
          ...
          onEnterClick(val.result, info, 'addDataRecordList').then(res => {
            if(res) {
    ...
    return (record.resultType === 'VALUE' || (['create', 'update'].includes(record._status) && record.$form.getFieldValue('valueType') === 'VALUE')) ? (
      <Form.Item className={styles[className]}>
        {record.$form.getFieldDecorator('result', {
          ...
          <Input className={`data-collection-modal-result-${index}`} onPressEnter={e => this.handleEnterClick(e.target.value, record, index)} />
        )}
      </Form.Item>
      ...
      <div className={styles[className]}>
        <Input disabled className={`data-collection-modal-result-${index}`} value={record.result} />
      </div>
    ) : record.result;
  }

  render() {
    const { dataSource = [], visible, loading, tenantId, modelName } = this.props;
    const { selectedRows } = this.state;
    const rowSelection = {
      columnWidth: 10,
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'center',
        render: (value, record) =>
          record._status === 'create' && modelName !== 'lotOperationPlatform' ? (
    ...
    return (
      <Modal
        destroyOnClose
        width={1100}
        title={modelName !== 'lotOperationPlatform' ? '补充数据采集' : '数据采集详情'}
        visible={visible}
        ...
        footer={(
          <Fragment>
            ...
            {modelName !== 'lotOperationPlatform' && (
              <Button
                type="primary"
