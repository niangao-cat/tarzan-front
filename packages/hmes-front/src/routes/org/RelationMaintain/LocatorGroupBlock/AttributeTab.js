...
export default class AttrTab extends PureComponent {
  ...
  componentDidMount() {
    const { dispatch, locatorGroupId } = this.props;
    if (locatorGroupId === 'create') {
      dispatch({
  ...
  render() {
    ...
    const columns = this.columns.map(col => {
      ...
      return {
        ...
        onCell: record => ({
          record,
          editable: locatorGroupId !== 'create' ? (editFlag ? false : col.editable) : col.editable,
          dataIndex: col.dataIndex,
          title: col.title,
          handleSave: this.handleSave,
          tableName: 'mt_mod_locator_attr',
        }),
      };
    });
    return (
