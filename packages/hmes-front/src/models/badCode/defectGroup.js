    ...
    *fetchSingleCode({ payload }, { call, put }) {
      ...
      if (list) {
        yield put({
          type: 'updateState',
          payload: {
            defectCodeItem: chainget(list, 'rows.mtNcGroup', {}),
            mtNcSecondaryCodeList: chainget(list, 'rows.mtNcSecondaryCodeList', []),
            mtNcValidOperList: chainget(list, 'rows.mtNcValidOperList', []),
            canBePrimaryCode: chainget(list, 'rows.mtNcGroup.canBePrimaryCode', true) !== 'N',
            limitSiteId: chainget(list, 'rows.mtNcGroup.siteId', undefined),
          },
        });
      }
      return list;
