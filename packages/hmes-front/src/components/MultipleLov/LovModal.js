...
class MultipleLovModal extends React.Component {
  constructor(props) {
    ...
    const { lov: { valueField: rowKey = defaultRowKey } } = this.props;
    if (records) {
   ...
   *
   *单击 选中/去选
   * @param {Object} record 当前选中的记录
   * @memberof MultipleLovModal
   * add by jing.chen04@hand-china.com
   */
  @Bind()
  handleRowClick(record) {
    ...
    const currentRowKey = this.parseField(record, rowKey);
    const flagIndex = selectedRowKeys.findIndex(key => {
    ...
    // 若存在，且被取消，则删除
    if (flagIndex > -1) {
      selectedRowKeys.splice(flagIndex, 1);
    }
    this.setState({
      selectedRowKeys,
    });
    selectedRowMap[currentRowKey] = record;
    const newSelectedRows = selectedRowKeys.map(selectedRowKey => selectedRowMap[selectedRowKey]);
    this.props.onSelect(newSelectedRows);
  }
  ...
  @Bind()
  dataFilter(data) {
    const { lov: { valueField: rowKey = defaultRowKey, childrenFieldName } } = this.props;
