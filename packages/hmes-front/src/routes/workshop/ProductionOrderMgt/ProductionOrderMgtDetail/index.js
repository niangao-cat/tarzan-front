...
export default class ProductionOrderMgtDetail extends Component {
  ...
  onSave = () => {
    ...
    dispatch({
      ...
      if (res && res.success) {
        ...
        history.push(`/hmes/workshop/production-order-mgt/detail/${res.rows}`);
  ...
  jumpPage=(record)=>{
    ...
    const workOrderId = match.params.id;
    history.push(`/hmes/workshop/production-order-mgt/execute-operation-management/detail/${record.eoId}/${workOrderId}`);
  }
          ...
          {attributeLists.map(attr => (
            <Fragment>
              <Card
                ...
                size="small"
              />
              <div style={{ display: attr.formType ? 'block' : 'none', backgroundColor: '#fff' }}>{attr.component}</div>
            </Fragment>
          ))}
          <Tabs
