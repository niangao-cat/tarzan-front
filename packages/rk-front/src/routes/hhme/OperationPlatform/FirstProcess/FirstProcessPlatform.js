...
import FirstProcessInfo from './FirstProcessInfo';
...
export default class FirstProcessPlatform extends Component {
  constructor(props) {
    super(props);
  ...
  handleInitData(isInit = true) {
    ...
    clearInterval(this.timeItemTimer);
    clearInterval(this.timer);
    if (this.firstProcessInfoForm) {
      this.firstProcessInfoForm.setFieldsValue({ labCode: undefined });
    }
  }
      ...
      const fetchBaseInfo = () => {
        ...
        dispatch({
          ...
          if (res) {
            ...
            if (this.firstProcessInfoForm) {
              this.firstProcessInfoForm.setFieldsValue({ labCode: res.labCode });
            }
  ...
  handleOutSite(info = {}) {
    ...
    payload = {
      ...
      ...info,
      ...payload,
      labCode: isEmpty(this.firstProcessInfoForm) ? null : this.firstProcessInfoForm.getFieldValue('labCode'),
  ...
  openBadHandTab() {
    ...
    const firstProcessfieldsValue = (this.firstProcessInfoForm && filterNullValueObject(this.firstProcessInfoForm.getFieldsValue())) || {};
    ...
    if (!isEmpty(barCodeSelectedRows)) {
      ...
      newMaterialSelectedRows.forEach(e => {
        ...
        if (record.materialId === e.materialId && record.lineNumber === e.lineNumber) {
          componentList.push({
            ...currentMaterial,
            virtualIdList: record.virtualIdList,
          });
        } else {
      ...
      return dispatch({
        ...
        payload: {
          ...
          barCodeSelectedRows,
          materialSelectedRows,
          useSourceFlag: record.useSourceFlag,
          isPumpProcess: 'N',
        },
      }).then(res => {
        if (res) {
  ...
  render() {
    ...
    const firstProcessInfo = {
      ...
      onRef: node => {
        this.firstProcessInfoForm = node.props.form;
      },
          ...
          >
            <Row gutter={48} style={{ height: 400 }}>
              <Col span={8} style={{ height: '100%', paddingLeft: '12px', paddingRight: '2px' }}>
                <FirstProcessInfo {...firstProcessInfo} />
              </Col>
