  ...
  effects: {
    ...
    *fetchEoList({ payload }, { call, put }) {
      ...
      if (list) {
        // 重构 背景色显示
        let index = 0; // 设置索引
        let eoBomName = ""; // 设置重复数据 变量
        // 判断是否不为空
        if(list.rows.content!==undefined&&list.rows.content!==null){
          for(let i=0; i<list.rows.content.length; i++){
            // 第一次时 直接赋值
            if(i !==0){
              // 判断 装配工单是否相同， 不同则新增
              if(list.rows.content[i].eoBomName !==eoBomName){
                index++;
                // eslint-disable-next-line prefer-destructuring
                eoBomName = list.rows.content[i].eoBomName;
              }
            }else{
              // eslint-disable-next-line prefer-destructuring
              eoBomName = list.rows.content[0].eoBomName;
            }
            list.rows.content[i].index = index;
          }
        }
        yield put({
          type: 'updateState',
