...
export default class Input3Form extends Component {
  constructor(props) {
    super(props);
    props.onRef(this);
    this.state = {
  ...
  onRadioChange = e => {
    ...
    const {
      ...
      setCurrentRule,
      changeRuleId,
      numberRange: { inputBox3List = {} },
    } = this.props;
    const { numrangeRuleId } = inputBox3List;
    setCurrentRule(3, e.target.value);
    changeRuleId(e.target.value, 2);
    dispatch({
      type: 'numberRange/updateState',
      payload: {
        inputBox3List: { numrangeRuleId },
      },
    });
    if (e.target.value === '1') {
      form.resetFields([
  ...
  onInputChange(rule, value, exampleValue, format) {
    ...
    setCurrentValue(3, value, exampleValue, format);
    if (rule === '5') {
