...
@connect(({ goldThread, loading}) => ({
  goldThread,
  createBarCodeLoading: loading.effects['goldThread/createBarCode'],
  loading: loading.effects['goldThread/queryBoxNo'],
  printLoading: loading.effects['goldThread/print'],
}))
export default class PasteChipArfterTable extends Component {
  ...
  @Bind()
  getBoxNo() {
   const { dispatch, goldThread: { workcellInfo= {}, defaultSite= {}, materialLot= {}}, setCheckedData} = this.props;
    dispatch({
      type: 'goldThread/queryBoxNo',
      payload: {
       ...workcellInfo,
       ...defaultSite,
       wafer: materialLot.wafer,
  ...
  addBarCode() {
   ...
   siteOutAndPrint() {
     ...
     // 执行出站
     const { dispatch, goldThread: { workcellInfo= {}, defaultSite= {}, materialLot= {}}, getSetOutData} = this.props;

     if(outData.length>0){
      dispatch({
        type: 'goldThread/setOut',
        payload: {
          ...workcellInfo,
          ...defaultSite,
          jobIdList: outData,
   ...
   */
  render() {
    const { loading, createBarCodeLoading, printLoading, goldThread: { containerType= [], boxNoList= {}, materialLot= {}}, form, setCheckedData, selectDataKeys } = this.props;
