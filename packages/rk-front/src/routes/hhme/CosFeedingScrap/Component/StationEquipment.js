...
export default class SerialItem extends Component {
  ...
  handleChangeColor(color) {
    const newColor = color === 'GREY' ? '#999' : color === 'BLUE' ? '#3C80F7' : color === 'GREEN' ? '#1AC0A6' : color === 'LIGHTGREEN' ? '#99CC66' : '';
    return newColor;
  }
      ...
      onFetchEqInfo(assetEncoding).then(res => {
        if(res) {
          if(onChange && data._status === 'update') {
            ...
            this.handleBindingEq(assetEncoding, { equipmentCategoryDesc, equipmentCategory }, true, index);
          }
        }
      });
    }
    ...
    if(onBinding) {
      onBinding({
        ...
        if(res && res.success === true) {
          ...
          const equipmentCodeDom = document.getElementsByClassName('equipment-code');
          if(equipmentCodeDom.length > 0) {
            if(equipmentCodeDom[index]!==undefined&&equipmentCodeDom[index]!==""&&equipmentCodeDom[index]!==null){
              equipmentCodeDom[index].focus();
              equipmentCodeDom[index].select();
            }
          }
        } else if(res && res.failed === true && isCreate) {
          notification.warning({
    ...
    const columns = [
      {
        ...
        render: (val, data) => !data.equipmentId ? (
          <Input
            onPressEnter={e => {
              this.handleEnterClick(e, data);
            }}
            className='asset-encoding'
          />
        ) : val,
      },
      {
                      ...
                      <Tooltip title={e.assetEncoding}>
                        ...
                        >
                          {e.assetEncoding ? e.assetEncoding : (
                            <Input onPressEnter={event => this.handleBindingEq(event.target.value, e, false, index)} className="equipment-code" />
                          )}
                        </div>
                      </Tooltip>
                    </div>
