...
import { queryAttrTL } from '../../services/api';

/**
 * 扩展字段多语言组件 AttrTLEditor
 *
 * @author wangjiacheng <jiacheng.wang@hand-china.com>
 * @extends {Component} - React.PureComponent
 * @reactProps {!boolean} [allowClear=false] - 是否允许清除
 * @reactProps {!string} [field=''] - 表单域
 * @reactProps {!string} [tableName=''] - tableName
 * @reactProps {!string} [keyId=''] - 主键id
 ...
 *
 */
export default class AttrTLEditor extends React.Component {
  input;
  ...
  save(data) {
    ...
    this.setState({ text: data[language], list: newList });
    this.setState({ modalVisible: false }, () => {
      this.input.focus();
    });
  ...
  @Bind()
  openModal() {
    const { field: fieldName = '', tableName = '', keyId = '' } = this.props;
    const { list, text, language } = this.state;
    this.showLoading(true);
    queryAttrTL({ fieldName, tableName, keyId })
      .then(res => {
        if (getResponse(res)) {
