...
export default class Input5Form extends Component {
  constructor(props) {
    super(props);
    props.onRef(this);
    this.state = {
  ...
  onRadioChange = e => {
    ...
    const {
      ...
      setCurrentRule,
      changeRuleId,
      numberRange: { inputBox5List = {} },
    } = this.props;
    const { numrangeRuleId } = inputBox5List;
    setCurrentRule(5, e.target.value);
    changeRuleId(e.target.value, 4);
    dispatch({
      type: 'numberRange/updateState',
      payload: {
        inputBox5List: { numrangeRuleId },
      },
    });
    if (e.target.value === '1') {
      form.resetFields([
  ...
  onInputChange(rule, value, exampleValue, format) {
    ...
    setCurrentValue(5, value, exampleValue, format);
    if (rule === '5') {
