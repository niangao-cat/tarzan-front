...
import {
  ...
  fetchNameplateList,
  printNameplateList,
} from '../../services/workshop/executeService';
  ...
  effects: {
    *init(_, { call, put }) {
      const res = yield call(queryMapIdpValue, {
        printComponentList: 'HME.PRINT_OPTIONS',
      });
      yield put({
        type: 'updateState',
        payload: {
    ...
    *print({ payload }, { call }) {
    ...
    // 打印条码validate
    *printSn({ payload }, { call }) {
      const res = getResponse(yield call(printSn, payload));
      return res;
    },

    // 打印条码校验
    *validate({ payload }, { call }) {
      const res = getResponse(yield call(validate, payload));
      return res;
    },
    ...
    *fetchNameplateList({ payload }, { call, put }) {
      const res = getResponse(yield call(fetchNameplateList, payload));
      if (res) {
        yield put({
          type: 'updateState',
          payload: {
            nameplateList: res.eoInternalCodeList.map(e => ({ ...e, _status: 'update' })),
            pdfDataList: res.pdfDataList,
          },
        });
      }
    },
    *printNameplateList({ payload }, { call }) {
      const res = getResponse(yield call(printNameplateList, payload));
      return res;
    },
  },
