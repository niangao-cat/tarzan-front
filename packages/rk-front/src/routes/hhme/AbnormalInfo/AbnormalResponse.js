...
export default class AbnormalResponse extends React.Component {
  ...
  handleCreate() {
    ...
    if (onCreate) {
      onCreate('lineList', 'linePagination', 'exceptionRouterId');
    }
  }
  ...
  handleCleanLine(record) {
    ...
    if (record._status === 'create') {
      if (onCleanLine) {
        onCleanLine('lineList', 'linePagination', 'exceptionRouterId', record);
      }
    } else if (onDelete) {
      onDelete(record, 'lineList', 'deleteLineList');
    }
  }
  ...
  handleEditLine(record, flag) {
    ...
    if (onEditLine) {
      onEditLine('lineList', 'exceptionRouterId', record, flag);
    }
  }
  ...
  render() {
    ...
    const columns = [
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('isTop', {
                ...
                <Checkbox
                  onChange={e => {
                    if (e.target.checked) {
                      record.$form.setFieldsValue({ upgradeTime: null });
                    }
                  }}
                />
              )}
      ...
      {
        title: intl.get(`${commonModelPrompt}.lastUpdatedUserName`).d('创建时间'),
        width: 120,
