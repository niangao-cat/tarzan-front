...
export default class DispatchDrawer extends PureComponent {
  ...
  handleCreate() {
    const {
      dispatch,
      relationMaintainDrawer: { proDispatchList = [], proDispatchPagination = {} },
    } = this.props;
    if (
  ...
  handleEdit(record, flag) {
    const {
      dispatch,
      relationMaintainDrawer: { proDispatchList },
    } = this.props;
  ...
  handleSave(record, index) {
    ...
    record.$form.validateFields((err, values) => {
      if (!err) {
        dispatch({
          type: 'relationMaintainDrawer/saveDispatchMethods',
          payload: {
  ...
  deleteData = (val, record) => {
    ...
    if (val && val !== '') {
      dispatch({
        type: 'relationMaintainDrawer/deleteDispatchMethods',
        payload: [val],
      }).then(res => {
        if (res && res.success) {
