...
@connect(({ reserveQuery, loading }) => ({
  reserveQuery,
  fetchMessageLoading: loading.effects['reserveQuery/queryBillList'],
}))
...
export default class FilterForm extends React.Component {
  ...
  fetchQueryList(pagination = {}) {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (!err) {
        dispatch({
          type: 'reserveQuery/queryBillList',
          payload: {
            ...fieldsValue,
            page: pagination,
          },
  ...
  changeOrgType(value) {
    ...
    const orgLovCode =
      value === 'LOCATOR' ? 'MT.LOCATOR_BY_ORGANIZATION' : 'MT.ORAGANIZATION_BY_TYPE';
    this.setState({
      orgTypeCode: value,
      orgLovCode,
      orgCode: '',
  ...
  render() {
    ...
    const { form, reserveQuery } = this.props;
    const { getFieldDecorator } = form;
    const { orgTypeList, ownerTypeList, holdTypeList, holdOrderTypeList } = reserveQuery;
    return (
      <Form className={SEARCH_FORM_CLASSNAME}>
          ...
          <Col {...FORM_COL_4_LAYOUT} className={SEARCH_COL_CLASSNAME}>
            <FormItem>
              <Button style={{ marginLeft: 8 }} onClick={this.handleQueryDetails}>
                {queryDetailsVisible
                  ? intl.get('tarzan.inventory.reserveQuery.button.retractSearch').d('收起查询')
                  : intl.get('tarzan.inventory.reserveQuery.button.moreSearch').d('更多查询')}
              </Button>
              <Button onClick={this.handleFormReset}>
                {intl.get('hzero.common.button.reset').d('重置')}
              </Button>
              <Button type="primary" htmlType="submit" onClick={this.fetchQueryList}>
                {intl.get('tarzan.inventory.reserveQuery.button.search').d('查询')}
              </Button>
            </FormItem>
          </Col>
        </Row>
        ...
        >
          <Col {...FORM_COL_4_LAYOUT}>
            ...
            >
              {getFieldDecorator('orderId')(<Input allowClear />)}
            </Form.Item>
          </Col>
          <Col {...FORM_COL_4_LAYOUT}>
            <Form.Item
            ...
            >
              {getFieldDecorator('holdType')(
                <Select allowClear>
                  {holdTypeList instanceof Array &&
                    holdTypeList.length !== 0 &&
                    holdTypeList.map(item => {
                      return (
                        <Select.Option value={item.typeCode} key={item.typeCode}>
                          {item.description}
                        </Select.Option>
          ...
          <Col {...FORM_COL_4_LAYOUT}>
            ...
            >
              {getFieldDecorator('orderType')(
                <Select allowClear>
                  {holdOrderTypeList instanceof Array &&
                    holdOrderTypeList.length !== 0 &&
                    holdOrderTypeList.map(item => {
                      return (
                        <Select.Option value={item.typeCode} key={item.typeCode}>
                          {item.description}
                        </Select.Option>
